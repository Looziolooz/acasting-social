...
import { X, Loader2, Download, Copy, MessageSquare, Sliders, CheckCircle2, Facebook, Instagram, Linkedin, AlertCircle, Send } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
      <div className="relative w-full max-w-5xl bg-surface-1 rounded-[32px] overflow-hidden border border-white/10 flex flex-col max-h-[95vh]">
        ...
        <div className="flex flex-col md:flex-row overflow-hidden">
          {/* Preview Panel */}
          <div className="md:w-[380px] p-8 bg-black/40 flex flex-col gap-5 border-r border-white/5 overflow-y-auto">
          ...
          <div className="flex-1 p-8 overflow-y-auto bg-surface-1">
            {step === 'preview' && (
              <div className="flex flex-col gap-8">
                <section>
                  <h3 className="text-xs font-bold text-white/40 uppercase mb-4 tracking-widest">Seleziona Stile</h3>
                ...
                {currentStyle === 'custom' && (
                  <section className="p-6 rounded-2xl bg-accent/5 border border-accent/20 flex flex-col gap-6 animate-slide-up">
                    <div className="flex items-center gap-2 text-accent-light text-xs font-bold uppercase"><Sliders size={16}/> Editor HD</div>
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between text-[10px] text-white/50 mb-2 uppercase">Posizione Titolo (Y): <span>{customSet.titleY}px</span></div>
                      ...
                      <div>
                        <div className="flex justify-between text-[10px] text-white/50 mb-2 uppercase">Luminosità Sfondo: <span>{customSet.brightness}</span></div>
                    ...
                    <button onClick={() => onRegenerate('custom', customSet)} className="w-full py-3 bg-accent text-white rounded-xl text-[11px] font-bold shadow-xl hover:brightness-110 transition-all active:scale-[0.98]">Applica Modifiche HD</button>
                  </section>
                )}
            ...
            {step === 'platforms' && (
              <div className="flex flex-col gap-6 animate-fade-in">
                <h3 className="text-xl font-bold text-white">Pubblicazione Social</h3>
                <div className="grid grid-cols-2 gap-3">
                  {(Object.entries(PLATFORM_CONFIG) as [Platform, any][]).map(([p, config]) => {
                    ...
                    return (
                      <button key={p} onClick={() => setSelectedPlatforms(prev => selected ? prev.filter(x => x !== p) : [...prev, p])}
                        ...
                        <div className="text-left text-sm font-bold text-white">{config.label}</div>
                      </button>
                    );
                  })}
                </div>
                <div className="flex gap-3 mt-4">
                  <button onClick={() => setStep('preview')} className="px-6 py-4 rounded-xl bg-surface-3 text-white font-bold text-xs uppercase">Indietro</button>
                  <button onClick={handlePublish} disabled={!selectedPlatforms.length} className="flex-1 py-4 rounded-xl bg-accent text-white font-bold text-xs uppercase tracking-widest shadow-xl">
                    Pubblica ora
                  </button>
                </div>
              </div>
            ...
            {step === 'publishing' && (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <h3 className="text-xl font-bold text-white">Invio in corso...</h3>
              </div>
            )}
            ...
            {step === 'done' && (
              <div className="flex flex-col gap-6 animate-slide-up">
                ...
                <h3 className="text-2xl font-bold text-white text-center">Fatto!</h3>
