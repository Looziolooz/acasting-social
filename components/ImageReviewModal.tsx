...
interface Props {
  ...
  generating: boolean;
  onRegenerate: (style: ImageStyle) => void;
        ...
        <div className="flex flex-col md:flex-row overflow-y-auto" style={{ maxHeight: 'calc(92vh - 68px)' }}>
            ...
            <div className="relative rounded-2xl overflow-hidden bg-black aspect-[9/16]"
              ...
              {generating ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 skeleton">
                  ...
                  <span className="text-white/60 text-sm">Generazione in corso...</span>
                </div>
              ) : imageUrl ? (
                <Image
                  src={imageUrl}
                ...
                <div className="absolute inset-0 flex items-center justify-center text-white/20 text-sm">
                  Seleziona uno stile per generare
                </div>
              )}
            </div>
          ...
          <div className="flex-1 p-5 flex flex-col gap-5 overflow-y-auto">
            ...
            {(step === 'preview') && (
              <>
                ...
                <div className="rounded-xl p-4" style={{ background: 'var(--surface-3)' }}>
                  ...
                  <dl className="space-y-2">
                    {[
                      ['Titolo', job.title],
                      ['Città', job.city || '—'],
                      ['Arvode', job.salary ? `${job.salary} kr` : 'Ej angivet'],
                      ['Scadenza', job.expiryDate ? job.expiryDate.split('T')[0] : 'Löpande'],
                ...
                <div className="flex gap-3 mt-auto pt-2">
                  <button
                    onClick={() => onRegenerate(currentStyle)}
                    disabled={generating}
            ...
            {step === 'platforms' && (
              <>
                <div>
                  <h3 className="font-display font-bold text-white mb-1">Dove pubblicare?</h3>
                  <p className="text-white/40 text-sm">Seleziona una o più piattaforme</p>
                </div>
                        ...
                        <button
                          ...
                          <div className="text-left">
                            ...
                            <div className={`text-xs mt-0.5 ${selected ? 'text-white/60' : 'text-white/30'}`}>
                              {selected ? 'Selezionato' : 'Click per aggiungere'}
                            </div>
                          </div>
                        </button>
                      );
                ...
                <div>
                  <label className="text-white/60 text-xs font-mono uppercase tracking-wider mb-2 block">
                    Caption personalizzata (opzionale)
                  </label>
                  <textarea
                    value={customCaption}
                    onChange={(e) => setCustomCaption(e.target.value)}
                    placeholder="Lascia vuoto per usare la caption generata automaticamente..."
                    rows={4}
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    style={{ background: 'linear-gradient(135deg, var(--accent), var(--accent-dark))', color: 'white' }}>
                    <Send size={14} />
                    Pubblica su {selectedPlatforms.length} piattaform{selectedPlatforms.length !== 1 ? 'e' : 'a'}
                  </button>
                </div>
              </>
            )}
            ...
            {step === 'publishing' && (
              <div className="flex flex-col items-center justify-center gap-4 py-12">
                ...
                <h3 className="font-display font-bold text-white text-xl">Pubblicazione in corso...</h3>
                <div className="flex gap-3">
            ...
            {step === 'done' && (
              <div className="flex flex-col gap-4">
                <div className="text-center py-6">
                  ...
                  <h3 className="font-display font-bold text-white text-xl">Operazione completata!</h3>
                </div>
                  ...
                  {publishResults.map((result) => (
                    <div key={result.platform}
                      ...
                      {result.success ? (
                        ...
                        <div className="flex items-center gap-1.5 text-red-400 text-xs" title={result.error}>
                          <AlertCircle size={14} />
                          Errore
                        </div>
                      )}
