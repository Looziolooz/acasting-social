            ...
            {showCaption && (
              <div className="p-5 bg-black/60 rounded-2xl border border-white/5 animate-fade-in">
                <div className="flex items-center justify-between mb-3 text-[10px] text-white/30 font-bold uppercase tracking-wider">
                  <span>Anteprima Manuale</span>
                  <button onClick={copyCaption} className="text-accent-light flex items-center gap-1 font-bold text-[10px]"><Copy size={12}/> Copia</button>
                </div>
          ...
          <div className="flex-1 p-8 overflow-y-auto bg-surface-1">
                ...
                {currentStyle === 'custom' && (
                  <section className="p-6 rounded-2xl bg-accent/5 border border-accent/20 flex flex-col gap-6 animate-slide-up">
                    ...
                    <div className="space-y-6">
                      <div>
                        <div className="flex justify-between text-[11px] text-white/50 mb-2 uppercase font-mono">Posizione Titolo (Y): <span className="text-accent-light font-bold">{customSet.titleY}px</span></div>
                      ...
                      <div>
                        <div className="flex justify-between text-[11px] text-white/50 mb-2 uppercase font-mono">Dimensione Font: <span className="text-accent-light font-bold">{customSet.titleSize}px</span></div>
                      ...
                      <div>
                        <div className="flex justify-between text-[11px] text-white/50 mb-2 uppercase font-mono">Luminosità: <span className="text-accent-light font-bold">{customSet.brightness}</span></div>
                    ...
                    <button onClick={() => onRegenerate('custom', customSet)} className="w-full py-4 bg-accent text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-xl">Applica Modifiche HD</button>
                  </section>
                )}

                <button onClick={() => setStep('platforms')} className="w-full py-5 rounded-2xl bg-white text-black font-black text-sm uppercase tracking-widest mt-4 shadow-2xl flex items-center justify-center gap-2">
            ...
            {step === 'platforms' && (
              <div className="flex flex-col gap-8 animate-fade-in">
                <div><h3 className="text-2xl font-display font-bold text-white mb-2 uppercase">Configurazione Social</h3><p className="text-white/40 text-sm">Seleziona le piattaforme di destinazione.</p></div>
                <div className="grid grid-cols-2 gap-4">
                  {(Object.entries(PLATFORM_CONFIG) as [Platform, any][]).map(([p, config]) => {
                    ...
                    return (
                      <button key={p} onClick={() => setSelectedPlatforms(prev => selected ? prev.filter(x => x !== p) : [...prev, p])}
                        className={`flex items-center gap-5 p-6 rounded-[24px] border-2 transition-all ${selected ? 'border-accent bg-accent/10' : 'border-white/5 bg-surface-2'}`}>
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center`} style={selected ? { backgroundColor: config.color } : { background: 'var(--surface-4)' }}><PlatformIcon platform={p} /></div>
                        <div className="text-left"><div className="font-bold text-white text-base">{config.label}</div><div className="text-[10px] text-white/30 uppercase">{selected ? 'Pronto' : 'Aggiungi'}</div></div>
                      </button>
                    );
                  })}
                </div>
                <div className="flex gap-4 mt-6">
                  <button onClick={() => setStep('preview')} className="px-8 py-4 rounded-2xl bg-surface-3 text-white font-bold text-xs uppercase tracking-widest">Indietro</button>
                  <button onClick={handlePublish} disabled={!selectedPlatforms.length} className="flex-1 py-5 rounded-2xl bg-accent text-white font-black text-sm uppercase shadow-2xl">Lancia su {selectedPlatforms.length} canali</button>
                </div>
              </div>
            )}
            ...
            {step === 'done' && (
              <div className="flex flex-col gap-8 animate-slide-up">
                <div className="w-24 h-24 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto border-2 border-emerald-500/30"><CheckCircle2 className="text-emerald-500" size={48} /></div>
                <div className="text-center"><h3 className="text-3xl font-display font-bold text-white mb-2 uppercase">Pubblicazione Riuscita!</h3></div>
                <div className="space-y-3">
                  {publishResults.map(r => (
                    <div key={r.platform} className="flex items-center justify-between p-5 bg-surface-2 rounded-2xl border border-white/5">
                      <div className="flex items-center gap-4"><div className="p-2 rounded-lg bg-surface-3"><PlatformIcon platform={r.platform} /></div><span className="text-sm font-bold text-white">{PLATFORM_CONFIG[r.platform].label}</span></div>
                      {r.success ? <span className="text-emerald-500 text-[10px] font-black uppercase flex items-center gap-1.5"><Check size={14}/> Successo</span> : <span className="text-red-500 text-[10px] font-black uppercase flex items-center gap-1.5"><AlertCircle size={14}/> Fallito</span>}
                    </div>
                  ))}
                </div>
                <button onClick={onClose} className="w-full py-5 rounded-2xl bg-surface-3 text-white font-black uppercase hover:bg-surface-4 transition-all">Chiudi Sessione</button>
              </div>
            )}
          </div>
        </div>
