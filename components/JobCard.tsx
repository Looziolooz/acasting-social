...
import { MapPin, Clock, Coins, Sparkles, SkipForward, CheckCircle2 } from 'lucide-react';
...
const STATUS_COLORS: Record<string, string> = {
  published: '#10b981', generated: '#F59E0B', skipped: '#6B7280',
};
const STATUS_LABELS: Record<string, string> = {
  published: 'Published', generated: 'Generated', skipped: 'Skipped',
};
        ...
        <div className="flex flex-col gap-1.5 mb-4">
          {job.city && (
            <div className="flex items-center gap-2 text-white/50 text-xs">
              <MapPin size={12} /><span>{job.city}</span>
            </div>
          )}
          {job.salary && (
            <div className="flex items-center gap-2 text-white/50 text-xs">
              <Coins size={12} /><span>{job.salary} kr</span>
            </div>
          )}
          {job.expiryDate && (
            <div className="flex items-center gap-2 text-white/50 text-xs">
              <Clock size={12} /><span>Deadline: {job.expiryDate.split('T')[0]}</span>
            </div>
          )}
        </div>
              ...
              {showStyles && (
                <div className="absolute top-full left-0 right-0 mt-2 rounded-xl overflow-hidden z-10 shadow-2xl"
                  ...
                  {(Object.entries(STYLE_LABELS) as [ImageStyle, typeof STYLE_LABELS[ImageStyle]][]).map(
                    ([styleKey, config]) => (
                      <button key={styleKey}
                        onClick={() => { setShowStyles(false); onGenerate(styleKey); }}
