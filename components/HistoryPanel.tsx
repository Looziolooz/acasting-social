...
export default function HistoryPanel() {
  ...
  if (!history.length) return (
    <div className="flex flex-col items-center justify-center py-24 text-white/20">
      ...
      <p className="font-display text-xl">No published posts yet</p>
      <p className="text-sm mt-2">Published jobs will appear here</p>
    </div>
  );
          ...
          <div key={item.id} className="rounded-2xl overflow-hidden card-hover"
            ...
            <div className="relative h-48" style={{ background: 'var(--surface-3)' }}>
              {item.generatedImage ? (
                ...
                <img src={item.generatedImage} alt={item.title} className="w-full h-full object-cover" />
              ) : (
                <div className="absolute inset-0 flex items-center justify-center">
                  <CheckCircle2 size={24} className="text-emerald-400 opacity-30" />
                </div>
              )}
              <div className="absolute inset-0"
                style={{ background: 'linear-gradient(to top, rgba(18,18,31,0.95) 0%, transparent 50%)' }} />
              <div className="absolute bottom-3 left-3 right-3">
                <p className="font-display font-bold text-white text-sm line-clamp-2 leading-tight">
                  {item.title}
                </p>
              </div>
            </div>
