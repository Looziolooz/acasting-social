...
export default function Dashboard() {
  ...
  const handleGenerate = async (
    ...
    try {
      ...
      console.log('📤 Generate payload:', {
        ...payload,
        originalImage: payload.originalImage?.substring(0, 80) + '...',
      });
  ...
  const handleRestore = async (jobId: string) => {
    try {
      ...
      if (res.ok) {
        setJobs((prev) =>
          prev.map((j) =>
            String(j.id) === jobId ? { ...j, processedStatus: 'pending' } : j
          )
        );
      }
    } catch (e) {
  ...
  const filteredJobs = jobs.filter((j) => {
    if (filter === 'new')
      return (
        !j.processedStatus ||
        j.processedStatus === 'pending' ||
        j.processedStatus === 'generated'
      );
    if (filter === 'done')
      return (
        j.processedStatus === 'published' || j.processedStatus === 'skipped'
      );
    return true;
  });
  ...
  const doneCount = jobs.filter(
    (j) => j.processedStatus === 'published'
  ).length;

  return (
    <div className="min-h-screen" style={{ background: 'var(--surface-0)' }}>
      <header
        className="border-b border-white/5 sticky top-0 z-40 backdrop-blur-xl"
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-display text-lg font-bold tracking-tight text-white">
              Acasting{' '}
              <span style={{ color: 'var(--accent-light)' }}>Social</span>
            </span>
          </div>
          ...
          <>
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-6">
                ...
                >
                  <Clock size={14} />
                  <span className="text-sm font-mono">{newCount} items</span>
                </div>
            ...
            {loading && !jobs.length ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="rounded-2xl skeleton h-64"
                    style={{
                      background: 'var(--surface-2)',
              ...
              <div className="flex flex-col items-center justify-center py-24 text-white/20">
                ...
                <p className="text-sm mt-2">
                  Click Refresh to load the latest listings
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {filteredJobs.map((job, i) => (
                  <JobCard
                    ...
                    onGenerate={(
                      style: ImageStyle,
                      custom?: CustomImageSettings
                    ) => handleGenerate(job, style, custom)}
      ...
      {selectedJob && (
        <ImageReviewModal
          ...
          onPublished={(platforms) =>
            handlePublished(String(selectedJob.id), platforms)
          }
          onClose={() => {
            setSelectedJob(null);
            setGeneratedImage(null);
