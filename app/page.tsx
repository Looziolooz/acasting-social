...
export default function Dashboard() {
  ...
  const [sourceQuality, setSourceQuality] = useState<'high' | 'medium' | 'low' | null>(null);
    ...
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          ...
          description: job.description, originalImage: alternativeImageUrl || job.imageUrl,
          style, customSettings: custom,
        }),
      });
  ...
  const handlePublished = (jobId: string) => {
    ...
    setSelectedJob(null); setGeneratedImage(null); setSourceQuality(null);
  };
  ...
  const handleRestore = async (jobId: string) => {
    try {
      ...
      if (res.ok) setJobs((prev) => prev.map((j) => String(j.id) === jobId ? { ...j, processedStatus: 'pending' } : j));
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--surface-0)' }}>
          ...
          <>
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-6">
                <div className="text-white/40 text-sm font-mono flex items-center gap-2"><Layers size={14} /><span>{jobs.length} total</span></div>
                <div className="flex items-center gap-2" style={{ color: 'var(--accent-light)' }}><Clock size={14} /><span className="text-sm font-mono">{newCount} new</span></div>
                <div className="text-emerald-400 flex items-center gap-2"><CheckCircle2 size={14} /><span className="text-sm font-mono">{doneCount} published</span></div>
              </div>
              <div className="flex items-center gap-2">
                {(['all', 'new', 'done'] as const).map((f) => (
                  <button key={f} onClick={() => setFilter(f)}
                    ...
                    style={filter === f ? { background: 'var(--surface-4)', border: '1px solid rgba(124,58,237,0.5)' } : { background: 'var(--surface-2)', border: '1px solid transparent' }}>
            ...
            {loading && !jobs.length ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {Array.from({ length: 6 }).map((_, i) => (<div key={i} className="rounded-2xl skeleton h-64" style={{ background: 'var(--surface-2)' }} />))}
              </div>
            ) : filteredJobs.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-white/20">
                <LayoutGrid size={48} className="mb-4 opacity-30" /><p className="font-display text-xl">No listings found</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {filteredJobs.map((job, i) => (
                  <JobCard key={job.id} job={job} index={i}
                    ...
                    onSkip={() => handleSkip(String(job.id))} onRestore={() => handleRestore(String(job.id))} />
                ))}
              </div>
            )}
          </>
      ...
      {selectedJob && (
        <ImageReviewModal job={selectedJob} imageUrl={generatedImage} currentStyle={currentStyle}
          generating={generating} sourceQuality={sourceQuality}
          onRegenerate={(style, custom, altUrl) => handleGenerate(selectedJob, style, custom, altUrl)}
          onPublished={(platforms) => handlePublished(String(selectedJob.id))}
          onClose={() => { setSelectedJob(null); setGeneratedImage(null); setSourceQuality(null); }} />
      )}
    </div>
  );
}